...
class HDF_Net(nn.Module):
    ...
    def forward_features(self,x,shape=None):
    ...
    def forward(self, image: torch.Tensor, mask, shape=None, *args, **kwargs):
        logits_1, logits_2, logits_3 = self.forward_features(image)
        mask_pred = torch.sigmoid(logits_1)
        loss = total_loss(logits_1, mask) + total_loss(logits_2, mask) / 4 + total_loss(logits_3, mask) / 8
        output_dict = {
            # loss for backward
            "backward_loss": loss,
