...
from dfformer.registry import DATASETS


@DATASETS.register_module()
class AbstractDataset(Dataset):
    def _init_dataset_path(self, path):
        path = None  # Tampered image path
        raise NotImplementedError  # abstract dataset!

        return path  # returns should be look like this
    ...
    def __init__(self, path,
        ...
        self.sample_list = self._init_dataset_path(path)
        ...
        # Edge mask generator
        self.edge_mask_generator = None if edge_width == 0 else EdgeMaskGenerator(
            edge_width)  # edge_with type is int,default is 0
    ...
    def __getitem__(self, index):
        ...
        tp_path, gt_path, _ = self.sample_list[index].split()
        # pil_loader or jpeg_loader
        tp_img = self.img_loader(tp_path)
        # shape, here is PIL Image
        tp_shape = tp_img.size
        # if gt is "0" then gt is a image with all 0
        if gt_path == "None":
            #temp = np.array(tp_img)
            gt_img = np.zeros((tp_shape[1], tp_shape[0], 3))
            label = 0
        else:
        ...
        tp_shape = crop_img.shape[0:2]  # H, W, 3 去掉最后一个3
        ...
        res_dict = self.post_transform(image=crop_img, masks=masks_list)
        tp_img = res_dict['image']  # H W -> 1 H W \
        gt_img = res_dict['masks'][0].unsqueeze(0)  # H W -> 1 H W \

        if self.edge_mask_generator != None:
            gt_img_edge = res_dict['masks'][1].unsqueeze(0)  # H W -> 1 H W
        else:
            gt_img_edge = torch.zeros_like(res_dict['masks'][0]).unsqueeze(0)
