...
class ViT(timm.models.vision_transformer.VisionTransformer):
    def __init__(self, input_head=None, num_channels=3,pretrained=True):
        ...
        if input_head != None:
            ...
            self.feature_patch_embed = nn.Conv2d(num_channels, original_first_layer.out_channels,
    ...
    def forward(self, image, mask, *args, **kwargs):
        if self.input_head != None:
            ...
            feature_embed = self.feature_patch_embed(feature)
            feature_embed = feature_embed.reshape(feature_embed.shape[0],feature_embed.shape[1],-1).permute(0,2,1)
            x = self.patch_embed(image)
            x = torch.concat([x,feature_embed],dim=1)
        else:
            x = self.patch_embed(image)
        x = self._pos_embed(x)
        x = self.blocks(x)
        feature = self.norm(x)
        if self.input_head != None:
            h = w = int((feature.shape[1]//2)**0.5)
            feature = (feature.permute(0,2,1)[:,:,1:1025] + feature.permute(0,2,1)[:,:,1025:]).reshape(image.shape[0],768,h,w)
        else :
